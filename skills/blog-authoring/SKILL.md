...
5. **配置信息准确性（必须执行）**：涉及软件安装步骤、配置参数、环境要求等技术细节时，**必须查找官方文档（GitHub README、官方文档站点）作为依据**，不可凭记忆或猜测编写。配置格式、命令参数、版本号等信息必须与官方保持一致。如果官方文档信息不完整，应注明「根据官方文档/实践验证」而非直接声称「官方推荐」
...
6. **信息说明检查**：
   - 「信息说明」中是否误加入了 **AI 配图说明**（生成方式、风格意象、`assets/prompt/` 路径等）；若有，应删除或改写入正文/图注
   - 是否仅保留事实溯源、引用链接、时效与免责等必要项
...
- **不要**在「信息说明」中写入与 **AI 配图** 相关的解释性内容，包括但不限于：是否由 AI 生成、画风/意象解读、为何选该图、以及指向 `image_prompt` / `image_prompt_file` / `assets/prompt/...` 的路径说明。这些信息已在仓库内的 front matter 与提示词文件中体现，重复写入文末会显得冗余，且偏离「信息来源」的本意。
- 若需在文中点明配图与主题的关联，应放在**正文**（例如首图旁的短句、小标题下的承接句、或图片 `alt` 文案），而不是塞进「信息说明」。
- 「信息说明」**仅宜**保留：外部引用与链接、数据/政策/产品的可核查出处、个人经历类条目的时间与情境说明、信息截止时间、免责声明（如法律/医疗不构成专业意见）等**与事实溯源或责任边界**直接相关的内容。
...
- [ ] 涉及时事的内容已标注信息截止时间
- [ ] **无法确认的事实信息已删除，不可猜测填写**
- [ ] 「信息说明」**未包含** AI 配图解释或提示词文件路径说明（此类信息以 front matter + 仓库内 `assets/prompt/` 为准）
