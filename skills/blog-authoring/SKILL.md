...
**前提**：仅使用 `_data/quotes.yml` 中存在的引用，禁止使用不在数据库中的引用。

1. **读取可用引用**：
   查看 `_data/quotes.yml`，筛选满足以下条件的引用：
   - `last_used` 为 `null`（从未使用），或
   - `last_used` 距今**超过30天**（即 `last_used < 当前日期 - 30天`）

2. **按类别选择**：
   根据文章主题从对应类别选择（如技术文优先「科学思想」「技术/商业」）
   - 优先选择 `used_count` 最少的引用（打破相同来源的重复）
   - 同一来源（如同一古典文献）不应在多篇文章中连续使用

3. **标记已用**（必须立即执行）：
   选定引用后，**立即**更新 `_data/quotes.yml` 中该引用的：
   - `used_count + 1`
   - `last_used` 改为今天（格式：`YYYY-MM-DD`）
...
**重要**：
- 每篇文章**仅使用一个引用**作为导语
- 引用的文字必须与 `quotes.yml` 中的 `text` 字段**完全一致**
- 禁止自行创作或修改引用文字
